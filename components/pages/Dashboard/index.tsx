...
import {
	use_unstableBreakpoint,
	useRowHeight,
} from "@/components/pages/Dashboard/hooks";
...
import {
	useDashboardItemIds,
	useSwitchCurrentDashboard,
} from "@/store/features/dashboards";
import store from "@/store/store";
import {
	breakpoints,
	cols,
	getFirstBreakpointValue,
	PersistedLayout,
} from "@/utils/layout";
...
function Dashboard() {
	...
	const handleLayoutChange = useRefCallback(
		(currentLayout: Layout[], layouts: Layouts) => {
			setLayouts(layouts);
...
const WidgetComponentMeno = memo(
	withSuspense(WidgetComponent, (ref: ForwardedRef<HTMLDivElement>) => (
		<div
			className="w-full h-full flex items-center justify-center text-xl text-gray-400 gap-2"
			ref={ref}
		>
